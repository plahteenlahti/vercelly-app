...
const catchNetworkError = (requestType: requestType, url: string) => (
  error: Error
) => {
  if (error.message === 'Network request failed') {
    throw new NetworkError({
      message: `Fetch ${requestType} to '${url}' failed, Network request failed`,
      statusCode: 500,
      errorCode: 500
    })
  }
...
const throwError = (
  ...
  throw new HttpError({
    message: `Fetch ${requestType} to '${response.url}' failed, status ${response.status} - '${response.statusText}' - '${response.bodyUsed}'`,
    statusCode: response.status,
    errorCode
  })
}

const getUrl = (config: string) => 'https://Vercelly/remote-config'
