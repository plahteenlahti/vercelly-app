...
export const loadRemoteConfig = async <T>(
  ...
  try {
    ...
    if (!cachedRemoteConfig) {
      ...
      loadAndStoreRemoteConfig(id) // This promise is intentionally ignored as the value is available on next request
...
const loadAndStoreRemoteConfig = async <T>(id: string): Promise<T> => {
  ...
  __DEV__ && console.log('Loaded remote config ', config)
  if (config) {
